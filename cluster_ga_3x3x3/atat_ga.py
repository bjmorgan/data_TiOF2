...
def next_generation( population, fitness_function ):
    ...
    while len( new_population ) < n_offspring:
        ...
        if c not in new_population:
            new_population += c
    selected = Population( new_population.boltzmann( fitness_function, temp=300.0, size=100 ) )
    return selected
...
if __name__ == '__main__':
    niter = 1000
    expansion = np.array( [ 3, 3, 3 ] )
    ref_eng = ref_energy()
    n_atoms = 3
    p1 = Population()
    args = ( n_atoms, expansion, ref_eng )
    a = np.array( [ 1 ] * 27 + [ 0 ] * 54 )
