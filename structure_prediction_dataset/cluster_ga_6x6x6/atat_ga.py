...
def next_generation(population, fitness_function, constraint):
    population.sort()
    best = population[0]
    n_offspring = 120
...
if __name__ == '__main__':
    a, b, c = (6, 6, 6)
    composition = {1:a*b*c, 0:a*b*c*2}
    niter = 500
    expansion = np.array([a, b, c])
    ref_eng = ref_energy()
    n_atoms = 3
    p1 = Population()
    args = ( n_atoms, expansion, ref_eng )
    a = np.array([1] * composition[1] + [0] * composition[0])
    ...
    for i in range( niter ):
        p2 = next_generation(p1, f, constraint=composition)
        p1 = p2
